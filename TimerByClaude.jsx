...
export function EnhancedTimer() {
  ...
  const handleTimeInput = useCallback(
    (field) => (e) => {
      const value = Math.max(
        0,
        Math.min(parseInt(e.target.value, 10) || 0, field === 'hours' ? 99 : 59)
      );
  ...
  const handlePauseResume = useCallback(() => {
    setStatus((prevStatus) =>
      prevStatus === TimerStatus.RUNNING
        ? TimerStatus.PAUSED
        : TimerStatus.RUNNING
    );
  }, []);
  ...
  const progressPercentage = useMemo(() => {
    return targetTimeInSeconds > 0
      ? (elapsedTime / targetTimeInSeconds) * 100
      : 0;
  ...
  return (
    <Card className="w-[350px]">
      <CardHeader>
        ...
        <CardDescription>
          Set your target time and start counting up
        </CardDescription>
      </CardHeader>
      <CardContent>
        <div className="mb-4">
        ...
        <div className="grid grid-cols-3 gap-2 mb-4">
          <Input
            ...
            onChange={handleTimeInput('hours')}
            min="0"
            max="99"
          />
          <Input
            ...
            onChange={handleTimeInput('minutes')}
            min="0"
            max="59"
          />
          <Input
            ...
            onChange={handleTimeInput('seconds')}
            min="0"
            max="59"
          />
        </div>
      ...
      <CardFooter className="flex justify-between">
        ...
        <Button
          onClick={handleStart}
          disabled={status !== TimerStatus.STOPPED}
          className="w-1/3"
        >
          Start
        </Button>
